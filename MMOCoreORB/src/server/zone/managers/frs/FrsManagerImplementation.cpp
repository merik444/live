...
void FrsManagerImplementation::promotePlayer(CreatureObject* player) {
	...
	info("FRS: Player: " + player->getFirstName() + " has enough xp to rank, promoting to rank: [" + String::valueOf(newRank) + "]",true);
...
int FrsManagerImplementation::calculatePvpExperienceChange(CreatureObject* attacker, CreatureObject* victim, float contribution, bool isVictim) {
	...
	info("Entered FRS XP calculations for attacker: " + attacker->getFirstName() + " Defender: " +  victim->getFirstName() + "Contribution values is:" + String::valueOf(contribution));
