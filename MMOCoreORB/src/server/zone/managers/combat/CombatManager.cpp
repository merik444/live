	...
	if (damage > 0) {
		...
		if (def == "saber_block") {
			if (!(attacker->isTurret() || weapon->isThrownWeapon()) && (weapon->isHeavyWeapon() || weapon->isSpecialHeavyWeapon() || weapon->getAttackType() == SharedWeaponObjectTemplate::RANGEDATTACK)) {
				...
				if (attacker->isPlayerCreature() && saberBlock > 50) { //saber block maximums in PVP.   85% after hard cap (MLS) with FRS
	
					switch (baseSaberBlock) {
						case 90:
							saberBlock = 65 * saberFRSMod;					// MLS up to 97.5 for Guardian and up
							break;
						case 85:
							saberBlock = 60 * saberFRSMod;					// MLS up to 90.0 
							break;	
						case 65:
							saberBlock = 55 * saberFRSMod;					// non-MLS Jedi Guardian and up 82.5% before MoM Debuff
							break;	
						case 60:
							saberBlock = 50 * saberFRSMod;					// non-MLS below Jedi Guardian up to 75% before MoM Debuff
							break;
						default:								// below 50 lightsaber block
							break;
					}
				}
...
/* Disabling stacking 2020/01/31	else if (!targetCreature->hasBuff(BuffCRC::SKILL_BUFF_HUNTED2)){
