...
int PlayerManagerImplementation::awardExperience(CreatureObject* player, const String& xpType,
	...
	if (amount <= 0 || xpType == "jedi_general" || xpType == "gcw_currency_rebel" || xpType == "gcw_currency_imperial" || xpType == "force_rank_xp" ){
			if (xpType == "force_rank_xp")
				xp = playerObject->addExperience(xpType, amount * globalFRSExpMultiplier);
			else
...
int PlayerManagerImplementation::calculateAttackers(CreatureObject* victim, CreatureObject* killer, ThreatMap* threatMap){
	...
	for (int i = 0; i < threatMap->size(); ++i) { //we should have been passed the victim's threatmap here as initial arguments for the function
		...
		info("FRS: calculateAttackers: Info on participant: [" + attacker->getFirstName() + "]");

		if (attacker->isGrouped()){
			if (attacker->getGroupID() == groupId){
				info("FRS: calculateAttackers: Participant is in killer or defender's group, not counting as part of threatmap");
				continue; //dont double count group members
			}

			if (attacker->getZone() != victim->getZone()){
			info("FRS: calculateAttackers: " + attacker->getFirstName() + " is not in the same zone as the victim, skipping");
			continue;
			}
		        if (CombatManager::instance()->areInDuel(attacker, victim)){
		                info("FRS: calculateAttackers: Member and victim are in a duel, no FRS granted");
				continue;
		        }
		}
		    //    if (CombatManager::instance()->areInDuel(attacker, victim)){
		...
		if (!allowSameAccountPvpRatingCredit && defenderGhost->getAccountID() == attackerGhost->getAccountID()){
			info("FRS: calculateAttackers ThreatMap: Participant is another character on the same account, disallowing FRS Credit");
			continue;
		}
		...
		if (pos.distanceTo(attacker->getWorldPosition()) > 256){
			info("FRS: calculateAttackers ThreatMap: Participant is out of range: [" + String::valueOf(pos.distanceTo(victim->getWorldPosition())) + "]");
			continue;
		}
		info("FRS: calculateAttackers ThreatMap: Participant added to threatmap total.");
		count++;
		}
		...
		if (group == NULL){
			info("FRS: calculateAttackers Killer/Defender is ungrouped, bypassing group check");
			return count;
		}
		...
		for (int i = 0; i < group->getGroupSize(); i++) {
			...
			if (member->getFactionStatus() != FactionStatus::OVERT && !memberGhost->hasBhTef()){
				info("FRS: calculateAttackers GroupCheck: Participant is not overt or a BH with an active mission, not counting.");
			...
			info("FRS: calculateAttackers: " + member->getFirstName() + " is not in the same zone as the victim, skipping");
			continue;
			}
			...
			if (isSameFaction(victim, member)){
				info("FRS: calculateAttackers GroupCheck: Participant is same faction, skipping");
				continue; //Cant be same faction
			}

			if (pos.distanceTo(member->getWorldPosition()) > 256){
				info("FRS: calculateAttackers GroupCheck: Participant is out of range for credit - Distance: [" + String::valueOf(pos.distanceTo(member->getWorldPosition()))  + "]");
...
void PlayerManagerImplementation::awardPvpProgress(TangibleObject* killer, CreatureObject* destructedObject, bool performedDeathBlow = false) {
	...
	if (!destructedObject->isRebel() && !destructedObject->isImperial() && !killedGhost->hasBhTef()){
		info("FRS: PVP Victim is not factionally aligned or a BH, no FRS Granted");
		return; //The victim is not factionally aligned and is therefor granted no PVP progress
	}

	//Lets see how many were involved.
	info("FRS: Involved Attackers for the death of: " + destructedObject->getFirstName());
	int countAttackers = calculateAttackers(destructedObject,killerCreature,attackerThreatMap);
	info("FRS: Involved Defenders for the death of: " + destructedObject->getFirstName());
	int countDefenders = calculateAttackers(killerCreature,destructedObject,victimThreatMap);

	info("FRS: New FRS Kill [" + String::valueOf(countAttackers) + "] Attackers, [" + String::valueOf(countDefenders) + "] Defenders");
	...
	award += rankBonus; 
	info("FRS: Award: + Rank Bonus = " + String::valueOf(rankBonus));
	...
	info("FRS:  Award: ratio of the difference in group size calculated to: " + String::valueOf(sizeRatio));
	...
	info("FRS: Award: modfiiedAward = " + String::valueOf(modifiedAward));
	...
	if (group != NULL){//apply final penalty of up to 50% based on group size
		info("FRS: Killer is grouped, adjusting down for group members, maximum penalty of 1/2");
		float gSize = Math::min(10,group->getGroupSize());// get the group size, cap it at 10

		info("FRS: Award: groupSize value = " + String::valueOf(gSize));

		float groupPenalty = (gSize * 5)/100;
		info("FRS: Award: Group Size Penalty = " + String::valueOf(groupPenalty));
		finalAward = finalAward * groupPenalty;
		info("FRS: Award: After Group Size Penalty = " + String::valueOf(finalAward));
		if (finalAward < (modifiedAward/2)){
		info("FRS: Final payout calculated to less than half original value, increasing to half payout) New Value = " + String::valueOf(finalAward));
	...
	if (sizeRatio < 1){ //Killer won against overwhelming odds, they are granted a bonus of 25% for each additional attacker
		info("FRS: Victim Won against a larger attacking force, bonus applied.");
		finalAward += ((modifiedAward * .25)/sizeRatio);
		info("FRS: Award: +OverWhelming Bonus" + String::valueOf(finalAward));
	}

	bool awardAllowed = true;

	if (!allowSameAccountPvpRatingCredit && ghost->getAccountID() == killedGhost->getAccountID()){
		info("FRS: No killer award allowed, same account");
		awardAllowed = false;
	}

	if (killerCreature->getFactionStatus() != FactionStatus::OVERT){
		info("FRS: Killer is not Overt, no FRS granted");
		awardAllowed = false;
	}

	if (CombatManager::instance()->areInDuel(killerCreature, destructedObject)){
                info("Killer and victim are in a duel, no FRS granted");
                awardAllowed = false;
	}
	...
	if ((killerCreature->hasSkill("force_rank_light_novice") || killerCreature->hasSkill("force_rank_dark_novice"))){
		//This prevents tactics like using a BH to lock a teammate out of the greater GCW fights, this is a valid tactic and still works, but the rest of your teammates
	   	//not geting FRS for assisting the bounty hunter should help to remove the incentive for the behavior.
		if (killedGhost->hasBhTef() && !destructedObject->hasSkill("combat_bountyhunter_investigation_03")){
			info("FRS: <Victim has recently, or is actively being hunted by a BH, no FRS is awarded>");
	...
	Vector3 pos(destructedObject->getWorldPositionX(), destructedObject->getWorldPositionY(), 0);
		...
		if (group != NULL) {
			info("FRS: Award: Killer is grouped, paying out FRS to all eligible");
			for (int i = 0; i < group->getGroupSize(); i++) {
				CreatureObject* member = group->getGroupMember(i);
				PlayerObject* memberGhost = member->getPlayerObject();
				...
				info("FRS: " + member->getFirstName() + " is not in the same zone as the victim, skipping");
				continue;
				}

				info("FRS: Award: Start Check for Participant [" + member->getFirstName() +  "]");

				if (member == NULL || memberGhost == NULL || member == killerCreature || !member->isPlayerCreature() || member == destructedObject){
					info("FRS: Participant is Killer or Victim, skipping: " + member->getFirstName());
				continue; //dont count the killer twice, dont count pets, dont count the victim if they some how managed to hurt themselves
			      ...
			      if (CombatManager::instance()->areInDuel(member, destructedObject)){
			            info("FRS: Member and victim are in a duel, no FRS granted, skipping: " + member->getFirstName());
					continue;
			      }

				if (member->getFactionStatus() != FactionStatus::OVERT){
					info("FRS: Participant is not Overt, skipping: " + member->getFirstName());
				continue; //we're only interested in overts
 				}

				if (isSameFaction(destructedObject, member)){
					info("FRS: Participant is same faction, skipping, " + member->getFirstName());
 					continue; //Cant be same faction
				}

				if (pos.distanceTo(member->getWorldPosition()) > 256){
					info("FRS: " + member->getFirstName() + "is Out of range of " + destructedObject->getFirstName() + ", Distance: [" + String::valueOf(pos.distanceTo(member->getWorldPosition())) + "] meters");
	...
	info("FRS: Award: deduction XP calculated to 65% of modifiedAward = " + String::valueOf(deductionXp));

	if (sizeRatio >= 4){
	deductionXp = 0;
        info("FRS: Award: Overhwelming odds protection activated, no FRS XP Lost");
	} 
	...
	info("PVP Death: Zone = " + playerZone + ". Killer " + killerName + " coordinates (x=" + kX + ", y=" + kY + ", z=" + kZ + ").  Victim " + playerName + " coordinates (x=" + pX + ", y=" + pY + ", z=" + pZ + ").",true);
	...
	if (killerCreature->hasSkill("force_rank_light_novice") && destructedObject->hasSkill("force_rank_dark_novice")) {
		zBroadcast << "Light Jedi " << killerName << " has defeated" << " Dark Jedi " <<  playerName << " in the FRS";
		info("Light Jedi " + killerName + " has defeated" + " Dark Jedi " +  playerName + " in the FRS",true);

	} else if (killerCreature->hasSkill("force_rank_dark_novice") && destructedObject->hasSkill("force_rank_light_novice")) {
		zBroadcast << "Dark Jedi " << killerName << " has defeated " << " Light Jedi " <<  playerName << " in the FRS";
		info("Dark Jedi " + killerName + " has defeated" + " Light Jedi " +  playerName + " in the FRS",true);
	}
	else {
		zBroadcast << playerName << " was slain in the GCW by " <<  killerName;
		info(playerName + " was slain in the GCW by " + killerName, true);
	}
