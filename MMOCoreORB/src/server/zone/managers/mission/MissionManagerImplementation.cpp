...
bool MissionManagerImplementation::isBountyValidForPlayer(CreatureObject* player, PlayerBounty* bounty) {
	...
	int maxNumberOfMissions = 1; // 1 BH mission from Padawn to FRS rank 3

	/* if (creature->hasSkill("force_rank_dark_novice") || creature->hasSkill("force_rank_light_novice")) //1 hunters at FRS rank 0 - 3
		maxNumberOfMissions++; */

	if (creature->hasSkill("force_rank_dark_rank_04") || creature->hasSkill("force_rank_light_rank_04")) //2 hunters from rank 4 - 7
		maxNumberOfMissions++;

	if (creature->hasSkill("force_rank_dark_rank_08") || creature->hasSkill("force_rank_light_rank_08")) //3 hunters from rank 8 - 10
		maxNumberOfMissions++;

	if (creature->hasSkill("force_rank_dark_master") || creature->hasSkill("force_rank_light_master")) //4 hunters at rank 11
		maxNumberOfMissions++;
