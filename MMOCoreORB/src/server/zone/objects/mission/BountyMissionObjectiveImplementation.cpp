...
void BountyMissionObjectiveImplementation::complete() {
	...
	ManagedReference<CreatureObject*> tc = owner->getZoneServer()->getObject(mission->getTargetObjectId()).castTo<CreatureObject*>();

	if (tc != NULL){
		//Player was killed
		owner->getZoneServer()->getMissionManager()->updatePlayerBountyReward(mission->getTargetObjectId(), VisibilityManager::instance()->calculateReward(tc));

		//Reset Visibility
		Reference<PlayerObject*> ghost = tc->getSlottedObject("ghost").castTo<PlayerObject*>();
		...
		//VisibilityManager::instance()->logout(tc);

		}
	}
	else{
	error("Invalid target: TC is null in complete()");
	}
