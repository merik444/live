...
bool AiAgentImplementation::validateTarget(SceneObject* target) {
	...
	if (targetCreatureObject != NULL && targetCreatureObject->getSkillMod("avoid_incapacitation") > 0 && (!homeLocation.isInRange(asAiAgent(), 32) || !target->isInRange(asAiAgent(), 32))) //Mobs are not interested in chasing invincible targets.
		return false;
...
bool AiAgentImplementation::validateStateAttack(CreatureObject* target, unsigned int actionCRC) {
	...
	VectorMap<uint64, StateEffect>* effects = queueCommand->getStateEffects();
	...
	for (int i = 0; i < effects->size(); i++) {
		uint64 effectType = effects->elementAt(i).getKey();
...
bool AiAgentImplementation::isAttackableBy(CreatureObject* object) {
	...
	unsigned int attackerFaction = getFaction();
	unsigned int targetFaction = object->getFaction();
	int attackerFactionStatus = getFactionStatus();
	int targetFactionStatus = object->getFactionStatus();


	if (isPet()) {
		ManagedReference<CreatureObject*> attackerOwner = getLinkedCreature().get();
		if (attackerOwner == NULL) {
			return false;
		}
		attackerFaction = attackerOwner->getFaction();
		attackerFactionStatus = attackerOwner->getFactionStatus();

		ManagedReference<PetControlDevice*> pcd = getControlDevice().get().castTo<PetControlDevice*>();

		if (pcd != NULL && pcd->getPetType() == PetManager::FACTIONPET) {

			if (object->isNeutral())
				return false;

			if (!(object->isPlayerCreature() || object->isPet() || object->isVehicleObject()))
				return attackerOwner->isAttackableBy(object, true);

			if (object->isPet() || object->isVehicleObject()) {
				ManagedReference<CreatureObject*> objectOwner = object->getLinkedCreature().get();
				if (objectOwner == NULL) {
					return false;
				}
				targetFaction = objectOwner->getFaction();
				targetFactionStatus = objectOwner->getFactionStatus();
			}

			if (targetFactionStatus != FactionStatus::OVERT || attackerFactionStatus != FactionStatus::OVERT) {
				return false;
			}

			if (targetFaction != 0 && attackerFaction != 0) {
	...
	if (object->isPet() || object->isVehicleObject()) {
		...
		if (pcd != NULL && pcd->getPetType() == PetManager::FACTIONPET) {
			...
			if (!(isPlayerCreature() || isPet()))
				return isAttackableBy(objectOwner);
			...
			if (isPet()) {
				...
				attackerFaction = owner->getFaction();
				attackerFactionStatus = owner->getFactionStatus();
			}
		
			if (attackerFactionStatus != FactionStatus::OVERT || targetFactionStatus != FactionStatus::OVERT) {
				return false;
			}
	...
	targetFaction = object->getFaction();
