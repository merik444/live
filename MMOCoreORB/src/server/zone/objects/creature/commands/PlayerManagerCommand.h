...
public:
	static int executeCommand(CreatureObject* creature, uint64 target, const UnicodeString& arguments) {
		...
		if (command == "path") {
		...
		} else if (command == "setfrsxpmodifier") {
			if (!tokenizer.hasMoreTokens()) {
				sendSyntax(player);
				return 1;
			}
			...
			playerManager->setFRSExperienceMultiplier(multiplier);

			StringBuffer message;
			message << "FRS Experience now increased by " << multiplier << "x";
	...
	static void sendSyntax(CreatureObject* player) {
		if (player != NULL) {
			...
			player->sendSystemMessage("Syntax: /server playermanager [setfrsxpmodifier] [value]");
