...
bool CreatureObjectImplementation::isBoss(){

	return (getOptionsBitmask() & OptionBitmask::BOSS);
}
...
void CreatureObjectImplementation::addAttackDelay(uint32 mod, int del) {
		cooldownTimerMap->updateToCurrentAndAddMili("nextAttackDelay", del * 1000);
...
void CreatureObjectImplementation::activateHAMRegeneration(int latency) {
	...
	if (isKneeling())
        ...
        int actionEnhancer = getSkillMod("action_enhancer") * 10;
	...
	uint32 actionTick = (uint32) ceil((float) Math::max(0, (getHAM(CreatureAttribute::STAMINA) + actionEnhancer)) * 13.0f / 2100.0f * modifier); //action Enhancer boost
	...
	if (object->isAiAgent()) {
		...
		if (object->isPet()) {
			...
			if (pcd != nullptr && pcd->getPetType() == PetManager::FACTIONPET) {
				...
				if (owner->getFactionStatus() != FactionStatus::OVERT || getFactionStatus() != FactionStatus::OVERT) {
					return false;
				}

				if(getFaction() == 0 || owner->getFaction() == 0 || owner->getFaction() == getFaction()) {
					return false;
				}
			}
...
int CreatureObjectImplementation::getHuntedLevel(CreatureObject* player){
	int hunted = 0;
	...
	if (player->hasBuff(BuffCRC::SKILL_BUFF_HUNTED2))
		hunted++;
	if (player->hasBuff(BuffCRC::SKILL_BUFF_HUNTED3))
		hunted++;
	return hunted;
}
...
float CreatureObjectImplementation::calculateCostAdjustment(uint8 stat, float baseCost) {
        ...
        float actionEnhancer = getSkillMod("action_enhancer");
	if (actionEnhancer > 0){


		if (stat ==  CreatureAttribute::QUICKNESS || stat ==  CreatureAttribute::STAMINA){
			stat += actionEnhancer * 10; //Hidden boost to action
		}
	}
...
bool CreatureObjectImplementation::hasDotImmunity(uint64 dotType) {
